...
class PeriodsChain
  ...
  def valid?
    ...
    periods.each do |period|
      ...
      next unless period.match?(/^\d{4}$/) # annually period
  ...
  def add(new_period_type)
    if valid?
      ...
      when 'annually'
        @periods.push((@last_period[:Y]).to_s)
      else
...
if Date.parse(start_date)
  ...
  print 'Periods chain is valid: '
  p periods_chain.valid?
  puts 'Adding function testing: '
  periods_chain.add('daily')
  p periods_chain.periods
  periods_chain.add('monthly')
  p periods_chain.periods
  periods_chain.add('annually')
  p periods_chain.periods
  print 'Periods chain is valid: '
  p periods_chain.valid?
end
